...
import { createInvoice } from "../../utils/report.pdf.js";
...
export const reportPdf = catchError(async (req , res , next)=>{
  const country = await countryModel.find({},{name:1 , donationBoxes:1 , convoysNumber:1,volunteers:1})
  const pdf = await createInvoice(country);
  res.setHeader('Content-Type', 'application/pdf');
  res.setHeader("Content-Disposition",'inline; filename="reportPdf.pdf"');
  console.log(pdf);
  res.send(pdf)
})