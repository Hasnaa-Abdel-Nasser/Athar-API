...
export const usersignup=asyncHandler(
 ...
 const html = `<!DOCTYPE html>
    ...
    if (!await sendEmail({ to: email, subject: 'Confirmation-Email', html })) {
        ...
        //create user
            const { _id } = await userModel.create({ name,email,gender,phone,job,country, password: hashPassword })
...
export const charitysignup=asyncHandler(
    async (req,res,next)=>{
        ...
        if (await charityModel.findOne({ email: email.toLowerCase() }) || await userModel.findOne({ email: email.toLowerCase() })) {
            return next(new Error("Email exist", { cause: 409 }))
        }
